/**
 * v4: Direct path mirror — for each unprefixed src item with attachments,
 * write patched content to the same path in v2_tenants (which the deployed
 * backend reads). Skips items that don't exist in v2_.
 */
...
async function main(){
  const summary = { srcScanned:0, srcWithAtt:0, dstExists:0, dstMissing:0, planned:0, byKind:{}, imagesInjected:0, alreadyPatched:0 };
  const writes = [];
  const spaces = await db.collection(`${SRC}/spaces`).get();
  console.log(`src spaces: ${spaces.size}`);
  let processed = 0;
  for (const s of spaces.docs) {
    ...
    for (const sp of sps.docs) {
      ...
      for (const it of its.docs) {
        ...
        if (dst.meta?._imagesPatchedV4) { summary.alreadyPatched++; continue; }

        const p = dst.payload || {};
        const meta = { ...(dst.meta||{}), _imagesPatchedV4: true, _imagesCount: att.length };
        ...
        if (dst.type === 'material' && p.materialType === 'rich') {
          ...
          patchData = { meta, payload: { ...p, richContent: { ...rc, blocks } } };
  ...
  console.log('\ndone');
  writeFileSync('subhang-fix/dumps/migrate-images-v4-result.json', JSON.stringify(summary,null,2));
}
