...
function hasQuestionOrderCrash(errs: string[], body: string): boolean {
  ...
  return (
    hay.includes("questionorder is not iterable") || hay.includes("questionorder is not a function")
  );
}
...
test.describe.serial("Assign → Take → Notify authentic flow", () => {
  test("1. Teacher Priya — Algebra Foundations timed test", async ({ page }) => {
    ...
    await record(
      "teacher",
      "Algebra Foundations space editor (timed test)",
      page,
      "03-teacher-algebra-edit",
      {
  ...
  test("2. Student Aarav — /tests + Start Test (no questionOrder crash)", async ({ page }) => {
    ...
    await record(
      "student",
      "Linear Equations timed-test landing",
      page,
      "08-student-test-landing",
      {
        pass: hasStart,
  ...
  test("3. Parent Suresh — notifications for child test prep", async ({ page }) => {
    ...
    const hasTestNotif =
      /test assigned|assigned.*test|aarav|math|algebra|timed test|test prep/i.test(notifBody);
    const emptyNotif = /no notifications yet/i.test(notifBody);
    await record(
      "parent",
      "notifications (test assigned to child)",
      page,
      "15-parent-notifications",
      {
        ...
        notes: [
          ...
          hasTestNotif
            ? "test-prep notification visible"
            : "GAP: no test-prep notification visible",
        ],
        pageErrors: [...errs],
      }
    );
