...
async function waitForRouteReady(page: Page, timeoutMs = 25_000) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    if (body.trim().length >= 40 || (hasHeading && hasNav) || (hasHeading && body.trim().length >= 15)) {
...
test.describe("Authentic multi-app live QA", () => {
  ...
  test("Teacher Priya — login + core routes", async ({ browser }) => {
    ...
    try {
      ...
      await evaluateRoute(page, "teacher", "/ (post-login)", "teacher-01-login", collectors, (body, url) => {
      ...
      if (await classLink.count()) {
        ...
        if (href) await gotoAndEval(page, "teacher", base, href.replace(base, "").split("?")[0], "teacher-18-class-detail", collectors);
      } else {
        results.push({
          app: "teacher",
          route: "/classes/:id",
  ...
  test("Student Aarav — login + core routes", async ({ browser }) => {
    ...
    try {
      ...
      await settle(page, 1500);
      await evaluateRoute(page, "student", "/ (post-login)", "student-01-login", collectors, (body, url) => {
        if (/\/login/.test(url)) return "still-on-login";
        if (/access denied/i.test(body)) return "access-denied";
        return null;
      });
      ...
      for (let i = 0; i < Math.min(count, 8); i++) {
        ...
        if (href && /^\/spaces\/[^/]+$/.test(href.split("?")[0])) {
          await gotoAndEval(page, "student", base, href.split("?")[0], "student-11-space-viewer", collectors);
          drilled = true;
          break;
        }
      }
  ...
  test("Admin Greenwood — login + core routes", async ({ browser }) => {
    ...
    try {
      ...
      await settle(page, 1500);
      await evaluateRoute(page, "admin", "/ (post-login)", "admin-01-login", collectors, (body, url) => {
        if (/\/login/.test(url)) return "still-on-login";
        if (/access denied/i.test(body)) return "access-denied";
        return null;
      });
    ...
    try {
      if (!(await ensureBaseUp(base))) {
        results.push({
          ...
          status: "FAIL",
          url: base,
          notes: ["Parent server not reachable on :4571 — credentials exist in TEST_CREDENTIALS.md (suresh.patel@gmail.com) but app down"],
        });
        return;
      }
      ...
      await settle(page, 1500);
      await evaluateRoute(page, "parent", "/ (post-login)", "parent-01-login", collectors, (body, url) => {
        if (/\/login/.test(url)) return "still-on-login";
        if (/access denied/i.test(body)) return "access-denied";
        return null;
      });
