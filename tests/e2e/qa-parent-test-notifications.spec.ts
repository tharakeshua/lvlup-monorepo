 ...
 *
 * Prerequisites:
 *   - parent-web on :4571 (use http://localhost:4571 — may be IPv6-only)
...
const PARENT_URL = process.env.PARENT_URL ?? "http://localhost:4571";
...
async function loginParent(page: import("@playwright/test").Page) {
  ...
  // Step 1: school code
  const schoolCode = page.getByLabel(/school code/i);
  await expect(schoolCode).toBeVisible({ timeout: 20000 });
  await schoolCode.fill(TENANT);
