...
function ExamCard({ exam, onOpen }: { exam: ExamVM; onOpen: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={meta.released ? onOpen : undefined}
      disabled={!meta.released}
      className={`bg-card flex w-full flex-col gap-3 rounded-lg border p-4 text-left transition-shadow ${
        meta.released ? "cursor-pointer hover:shadow-sm" : "cursor-default"
      }`}
    >
        ...
        <div className="space-y-3">
          {filtered.map((exam) => (
            <ExamCard
              key={exam.id}
              exam={exam}
              onOpen={() => navigate(`/exams/${exam.id}/results`)}
            />
          ))}
        </div>
      )}
