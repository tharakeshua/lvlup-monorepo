  ...
  const renderQuestionButton = (qId: string, index: number) => {
    ...
    return (
      <button
        ...
        className={`relative h-10 w-10 rounded text-xs font-medium transition-all sm:h-9 sm:w-9 ${statusColors[status]} ${
          isCurrent ? "ring-primary ring-2 ring-offset-1" : ""
        }`}
      >
        {index + 1}
        {symbol && (
          <span className="absolute -right-0.5 -top-0.5 text-[8px] leading-none" aria-hidden="true">
            {symbol}
          </span>
        )}
      </button>
          ...
          {sectionGroups.map((group) => (
            <div key={group.sectionId ?? "general"}>
              <div className="mb-1.5 flex items-center justify-between">
                <span className="text-foreground truncate text-xs font-medium">{group.title}</span>
      ...
      <div className="space-y-1 border-t pt-2">
        {Object.entries(statusLabels).map(([status, label]) => (
          <div key={status} className="flex items-center gap-2 text-xs">
            <span
              className={`inline-flex h-4 w-4 items-center justify-center rounded text-[8px] ${statusColors[status as QuestionStatus]}`}
            >
