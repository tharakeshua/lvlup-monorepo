...
## Charts (pure RN/SVG — `charts.tsx`, no heavy chart dep)

| Component         | Key props                                                                                                                                                             |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MiniBarChart`    | `data?:ChartDatum[]` **or** `values?:number[]`, `height?, color?, showValues?, showLabels?, maxValue?, onSelect?(i)` — `ChartDatum={value,label?,color?,active?}`     |
| `Sparkline`       | `values:number[], width?, height?, color?, fill?, showEnd?, strokeWidth?`                                                                                             |
| `DistributionBar` | `segments:{label?,value,color?}[], rows?, height?, showValues?` — `rows` ⇒ labelled bars (A–F grade letters auto-colored via `GRADE_COLORS`); else stacked single bar |

## Teacher composites (`teacher.tsx`)

| Component         | Key props                                                                                                                                                                                                           |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `RosterRow`       | `name, initials?, avatarUri?, roll?, progress?(0–100), score?:number\|null, grade?, status?:active\|inactive\|pending\|invited, atRisk?, lastActive?, selectable?, selected?, onToggleSelect?, onPress?, trailing?` |
| `SubmissionCard`  | `studentName, initials?, meta?, status?:awaiting\|auto-graded\|needs-review\|reviewed\|released\|flagged\|error, confidence?:high\|medium\|low, confidenceScore?(0–1), score?, maxScore?, onPress?`                 |
| `ConfidenceBadge` | `level?:high\|medium\|low, score?(0–1, auto-bands), hidePercent?, size?:sm\|md`                                                                                                                                     |
| `RubricRow`       | `criterion?:{label,earned?,max?,note?,state?:met\|partial\|missed}` or inline `label/earned/max/note/state`, `onPress?` (edit)                                                                                      |
| `RubricBreakdown` | `criteria:RubricCriterion[], totalEarned?, totalMax?, title?` (totals auto-summed)                                                                                                                                  |
| `ScoreInput`      | `value?:number\|null, onChange?(n\|null), max?, min?, step?, label?, disabled?, autoFocus?` — clamps; `step<1` ⇒ decimal keypad                                                                                     |
| `ScoreStepper`    | `value?, onChange?(n), min?, max?, step?, label?, disabled?` — +/- points                                                                                                                                           |
| `MetricCard`      | `label, value, caption?, delta?, trend?:up\|down\|flat\|na, icon?, tone?:neutral\|brand\|success\|warning\|error, visual?, onPress?`                                                                                |
| `AtRiskRow`       | `name, initials?, className_?, reasons?:ReactNode[], severity?:high\|medium\|low, score?:number\|null, trend?, lastActive?, flagged?, selectable?, selected?, onToggleSelect?, onPress?`                            |
| `RoleTenantPill`  | `tenant?, role?, code?, switchable?, onPress?` — top-bar tenant/role switcher trigger                                                                                                                               |
| `FilterChips`     | `options:{key,label,icon?,count?}[], value?:string\|string[], onChange?(key), multiple?, contentClassName?` — horizontal scroll                                                                                     |
