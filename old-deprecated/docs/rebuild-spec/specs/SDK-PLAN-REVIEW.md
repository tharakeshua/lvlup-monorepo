...
`keyof typeof QUERY_KEYS === DomainName` AND every INVALIDATION*GRAPH root ∈
DomainName, and make it pass against the \_real* inventory. Define
...
4. Re-run the 4 contract-integrity suites (registry, ACCESS*RULES completeness,
   INVALIDATION_GRAPH totality, transitions-enum) against the \_real* inventory
