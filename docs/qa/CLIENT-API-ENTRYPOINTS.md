...
Short map for web and future iOS/Android clients. Companion:
[`BACKEND-MOBILE-COMPAT.md`](./BACKEND-MOBILE-COMPAT.md).
...
**Client rule:** treat `CALLABLES[name].requestSchema` / `responseSchema` as the
only request/response shapes. Do not duplicate Zod in the app.
...
**Deploy id grammar:** nested export `v1.levelup.getSpace` → Firebase function
id `v1-levelup-getSpace`.

**Business logic:** stays in `@levelup/services`; sdk-v1 only wires
`makeCallable(name, serviceFn)`.
...
Transport maps dotted contract names → dashed deploy ids in `invokeViaCallable`
(`toDeployedCallableId`). Apps never call `httpsCallable` with dashed ids
directly.
