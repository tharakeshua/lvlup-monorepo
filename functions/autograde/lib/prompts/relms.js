...
exports.RELMS_SYSTEM_PROMPT = void 0;
exports.buildRELMSUserPrompt = buildRELMSUserPrompt;
exports.parseRELMSResponse = parseRELMSResponse;
exports.RELMS_SYSTEM_PROMPT = `You are RELMS, an expert exam answer evaluator. You grade student answers with precision, fairness, and structured feedback.
 ...
 */
function buildRELMSUserPrompt(question, rubric, enabledDimensions) {
  const criteriaBlock =
    rubric.criteria
      ?.map((c, i) => `  ${i + 1}. ${c.description ?? c.name} [${c.maxPoints} marks]`)
      .join("\n") ?? "  (No criteria defined)";
 ...
 */
function parseRELMSResponse(text, maxMarks) {
  const cleaned = text
    .replace(/^```(?:json)?\n?/m, "")
    .replace(/\n?```$/m, "")
    .trim();
