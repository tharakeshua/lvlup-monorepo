...
export async function sendNotification(payload: NotificationPayload): Promise<string> {
  ...
  await rtdb.ref(`${rtdbPath}/unreadCount`).transaction((current: number | null) => {
    return (current ?? 0) + 1;
  });
  ...
  logger.info(`Notification sent: ${payload.type} to ${payload.recipientId}`);
  return notifRef.id;
}
...
export async function sendBulkNotifications(
  ...
  for (let i = 0; i < recipientIds.length; i += BATCH_SIZE) {
    ...
    const incrementPromises = chunk.map((recipientId) => {
      ...
      return rtdb.ref(countPath).transaction((current: number | null) => (current ?? 0) + 1);
    });
    await Promise.all(incrementPromises);
  }

  logger.info(`Bulk notifications sent: ${sent} recipients for ${basePayload.type}`);
  return sent;
}
